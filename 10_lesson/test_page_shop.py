...
def test_shop():
    ...
    with allure.step("Создается объект класса PageAuth со \
    взаимодествием  \
    ...
    with allure.step("ОСоздается объект класса MainPageShop \
    со взаимодествием  \
    с браузером через driver"):
        page_selection = MainPageShop(driver)
    with allure.step("Выбираем три вида соусов и переносим \
    в корзину для оплаты"):
        page_selection.souce_selection()
    with allure.step("Определяем количество товара по \
    счетчику на странице корзины"):
        page_selection.transit_basket()
    with allure.step("Создаем объект класса BasketPageShop \
    со взаимодействем с  \
    браузером через driver"):
        page_basket = BasketPageShop(driver)
    with allure.step("Выводим в переменую значение списка  \
    товаров и считаем их количество"):
        res = page_basket.result_basket()
    with allure.step("Проверяем результат посчета товаров в  \
    корзине с заданным числом"):
        assert res == 3
    with allure.step("Подтверждаем выбор товара и отправляем \
    его на страницу оплаты"):
        page_basket.checkout()
    with allure.step("Создаем объект класса BuyerDataPage \
    со взаимодействием с браузером через driver"):
    ...
    with allure.step("Проверяем соответствие результата \
    переменной заданному значению суммы"):
